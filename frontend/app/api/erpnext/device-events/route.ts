...
export async function GET(request: NextRequest) {
  try {
    ...
    const deviceEui = searchParams.get("device_eui");

    if (!deviceEui) {
      return new Response(
        JSON.stringify({ message: "device_eui parameter is required" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        }
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({
          message: errorText || "Failed to stream device events",
        }),
        {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          console.error("Error streaming device events:", error);
          try {
    ...
    console.error("ERPNext device events proxy error:", error);
    return new Response(
      JSON.stringify({
        message:
