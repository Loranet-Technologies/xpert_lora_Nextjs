...
export async function POST(request: NextRequest) {
  try {
    ...
    // Forward to ERPNext
    const response = await fetch(ERPNEXT_API_URLS.RESET_USER_PASSWORD, {
      method: 'POST',
      headers,
    ...
    if (!response.ok) {
      ...
      const errorMessage = parseErpNextError(errorText, 'Failed to reset password');
      
      console.error(
        `Failed to reset password: ${response.status} - ${errorMessage}`,
        { errorText }
      );
    ...
    console.error('ERPNext reset password proxy error:', error);
    return NextResponse.json(
      {
